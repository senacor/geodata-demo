...
angularjsApp.factory('neighboursFactory', ['$http', function ($http) {
    var factory = {};
    factory.getNeighbours = function (country) {
        return $http.get('http://api.geonames.org/neighboursJSON?formatted=true&geonameId=' + country.geonameId + '&username=mygeo');
    };
    return factory;
}]);
...
/**
 * @ngdoc function
 * @name angularjsApp.controller:ChartsCtrl
 * @description
 * # ChartsCtrl
    ...
    .controller('ChartsCtrl', function ($scope, neighboursFactory) {
        ...
        $scope.handleChartClick = function (event) {
            ...
            if ($scope.chart) {
                ...
                var selectedSegment = event.country;
                var numberOfCountries = $scope.data.length;
                for (var i = 0; i < numberOfCountries; i++) {
                    if ($scope.data[i].label === selectedSegment.name) {
                        $scope.country = $scope.data[i].country;
                        loadChartData($scope.country);
                        break;
                    }
                }
            }
        ...
        $scope.options = {
            ...
            legendTemplate: '<ul class="tc-chart-js-legend"><% for (var i=0; i<segments.length; i++){%><li style="background-color:<%=segments[i].fillColor%>"><div ng-drag="true" ng-drag-data="{obj}" ng-drag-success="onDragComplete($data,$event)"><%if(segments[i].label){%><%=segments[i].label%><%}%></div></li><%}%></ul>'
        ...
        function loadChartData(country) {
            ...
            neighboursFactory.getNeighbours(country)
                ...
                .error(function (error) {
                    console.log('falied to load neighbours of ' + country.name);
                });
        }

        function createChartData(geonames) {
            var data = [];
        ...
        function getHighlightColor(index, countryName) {
            var i = index;
            if (index >= colors.length) {
                i = index - colors.length;
            }
            return colors[i].highlight;
        }

        // TODO das geht schöner
        function getCountryAreaInSqKm(geonameId) {
            for (var i = 0; i < countryInfo.length; i++) {
                if (countryInfo[i].geonameId === geonameId) {
                    return countryInfo[i].areaInSqKm;
                }
            }
            return 100;
        }

        // get it via api.geonames.org/countryInfoJSON?username=mygeo
        var countryInfo = [{
            'countryName': 'Andorra',
            'currencyCode': 'EUR',
            'fipsCode': 'AN',
